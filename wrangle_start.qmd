...
Python libraries are loaded with the `import` statement and can be aliased into simpler names with `as` at the same time. Note that common libraries (like Pandas) have a broadly-accepted abbreviations that are almost always used (like `pd` in the case of Pandas).
