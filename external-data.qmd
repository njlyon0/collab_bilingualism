...
We can now load an external dataset derived from the `lterdatasampler` `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r} package (see [here](https://lter.github.io/lterdatasampler/)) with both `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r} and `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py}. This relatively simple operation is also a nice chance to showcase how 'namespacing' (i.e., indicating which package a given function comes from) differs between the two languages.

:::panel-tabset
## `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` R

Namespacing in `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r} is accomplished by doing `package_name::function_name` and is optional (though, in my opinion, good practice!). Note that we use the assignment operator (`<-`) to assign the contents of the CSV to an object.
...
In `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py}, namespacing is required and is done via `package_name.function_name`. Note that we use the assignment operator (`=`) to assign the contents of the CSV to a variable.
...
In `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r}, such data are class `data.frame`
...
In `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py}, such data are type `DataFrame` and this variable type is defined by the `pandas` library. This is the standard type returned by the `pandas` `read_csv` function.
...
In `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r}, we use the `head` or `tail` function to return the first or last rows respectively.
...
In `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py}, we use the `head` or `tail` method to return the first or last rows respectively. Note that these methods are only available to variables of type `DataFrame`. All methods are appended to the end of the variable of the appropriate type separated by a period.
...
`r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r} uses the `str` function to assess data structure. Structure includes the dimensions of the data (i.e., number of rows and columns) as well as the class of the data (`data.frame`) and the class of each column.

```{r r-structure}
utils::str(vert_r)
```

Be careful to not confuse this with the `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py} function `str` that coerces values to type "string"!

## `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` Python

When we want to know the type of each column in a `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py} `DataFrame`, we can use the `dtypes` "attribute". Attributes are akin to a method but they completely lack arguments that might modify their behavior. As a consequence they are extremely precisely defined. The `dtypes` attribute returns the type of each column.
...
`r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r} provides the `summary` function to summarize all columns in a dataset. Note that it is not terribly informative for columns of class character.
...
`r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py} has the `describe` method for getting similar information about each column of a `DataFrame`.
...
`r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py} also offers the `info` method to identify only the count of non-null entries in each column as well as the type of each column.
...
`r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r} has the `names` function to quickly return a vector of the column names in a given `data.frame` object.
...
`r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py} has the `columns` attributes for returning a simple list of column labels in a given `DataFrame` variable.
...
We may want to access a particular column or set of columns. There are several approaches we might use to access a particular column in either `r fontawesome::fa(name = "python", fill = "#077DC2", a11y = "sem")` [Python]{.py} or `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r}. They are:
...
In `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r}, the column operator is a `$` and we place that character between the object and column names (e.g., `data_name$column_name`).
...
In `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r}, the column operator is a `.` and we place that character between the variable name and column label (e.g., `data_name.column_name`).
...
In `r fontawesome::fa(name = "r-project", fill = "#FF9B00", a11y = "sem")` [R]{.r} we could use a concatenated vector of index positions to specify particular columns by their positions.
