...
Some operations are only possible on some categories of information. For instance, we can only perform arithmetic on numbers. In Python this is known as the [variable]{.py}'s [type]{.py} & while in R this is the [object]{.r}'s [class]{.r}. In either case, it's important to know--and be able to check--this information about the [variables]{.py}/[objects]{.r} with which we are working.
...
One crucial difference between R and Python is that Python is "0-based" meaning that the first [item]{.py} is at index position `0` while in R the position of the equivalent [element]{.r} is `1`.
